...
1. Create a security group for the VPC Interface endpoints ([AWS docs](https://docs.aws.amazon.com/cli/latest/userguide/cli-services-ec2-sg.html)) 
```bash
...
```
2. Configure ingress policy for this SG (the egress is configured for 'free' when a new SG is created)
```bash
# Lookup CidrBlock 
...
```
3. Create VPC Interface endpoints
```
