...
class ProjExtensionFloatIntFix(PostgreSQLCommandRunner):
    """
    This class fixes the floating point values in proj:epsg and proj:shape properties
    (they should be int)
    """
    ...
    def sql_commands(self):
        ...
        epsg_update = """
            ...
            SET content = jsonb_set(
                content, '{properties, proj:epsg}',
                to_jsonb(CAST(
                    (content->'properties'->>'proj:epsg') AS INTEGER
                )), true
            )
        ...
        proj_shape_update0 = """
            UPDATE items
            SET content = jsonb_set(
                content, '{properties, proj:shape, 0}',
                to_jsonb(CAST(
                    (content->'properties'->'proj:shape'->0) AS INTEGER
                )), true
            )
        ...
        proj_shape_update1 = """
            UPDATE items
            SET content = jsonb_set(
                content, '{properties, proj:shape, 1}',
                to_jsonb(CAST(
                    (content->'properties'->'proj:shape'->1) AS INTEGER
                )), true
            )
